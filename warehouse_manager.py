...
class WarehouseManager:
    ...
    def __init__(self, path=DEFAULT_DATABASE_PATH):
        ...
        self.warehouses = [x[0] for x  in self.__execute_read_query(
                "SELECT name FROM sqlite_master WHERE type='table' and name!='forms_inputs';")]
    ...
    def add_input(self, input_name, input_type):
        ...
        table_query = f"""
            ALTER TABLE {self.current_warehouse}
                ADD {input_name} {input_type};
            """
        self.__execute_query(table_query)
    ...
    def __get_inputs(self):
        ...
        #     return None
        table_query = f"SELECT * FROM forms_inputs;"
        return [x[0] for x  in self.__execute_read_query(table_query)]
    ...
    def add_item(self, item_data):
        ...
        (name, quantity, other)
        VALUES ('{item_data["quantity"]}', {item_data["quantity"]}, '{item_data["other"]}')
        """
        self.__execute_query(table_query)
    ...
    def edit_item(self, id):
        ...
        input_data = self.__execute_read_query(
                f"SELECT * FROM forms_inputs;")[0]
        ...
        count = 2
        for column_value in input_data:
            self.edit_item_data = {
                column_value[count]
            }
            count+=1
    ...
    def delete_input(self, name):
        table_query = f"""
        DELETE FROM forms_inputs where Name = '{name}';
        """
        print(self.__execute_read_query(table_query))

        table_query = f"""
        ALTER TABLE {self.current_warehouse}
        DROP COLUMN {name};
        """
        print(self.__execute_query(table_query))